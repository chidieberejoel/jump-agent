...
config :jump_agent, JumpAgentWeb.Endpoint,
  ...
  code_reloader: true,
  debug_errors: true,
  secret_key_base: "fe1K5gpvaGyzYNTWffaJ1kmRAEtQMqhLsBJE0Ok/ogQ+u2WcarPWye/YWcYB6o+r",
  watchers: [
    esbuild: {Esbuild, :install_and_run, [:jump_agent, ~w(--sourcemap=inline --watch)]},
    tailwind: {Tailwind, :install_and_run, [:jump_agent, ~w(--watch)]}
  ]
...
config :jump_agent, JumpAgentWeb.Endpoint,
  live_reload: [
    patterns: [
      ...
      ~r"priv/gettext/.*(po)$",
      ~r"lib/jump_agent_web/(controllers|live|components)/.*(ex|heex)$"
    ]
  ]
...
config :langchain,
       ...
       default_timeout: System.get_env("LANGCHAIN_DEFAULT_TIMEOUT", "30000") |> String.to_integer()
